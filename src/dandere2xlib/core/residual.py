...
def make_residual_image(context: Context, raw_frame: Frame, list_residual: list, list_predictive: list):
    ...
    # Create a vector for every element in the residual's list. Each vector element is 4 lines long,
    # which accounts for the * 4's. Apply the vector to the out image.
    for x in range(int(len(list_residual) / 4)):

        # load the vector
        y = x * 4
        vector = list(map(int, list_residual[y:y + 4]))
